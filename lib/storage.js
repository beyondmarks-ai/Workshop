...
export async function listUsers() {
  const { service } = await storage();
  const users = [];
  for await (const item of service.getContainerClient("users").listBlobsFlat()) {
    if (!item.name.endsWith(".json")) continue;
    try { users.push(JSON.parse((await service.getContainerClient("users").getBlobClient(item.name).downloadToBuffer()).toString("utf8"))); } catch {}
  }
  return users;
}

export async function getUserByApiKey(apiKey) {
  if (!/^bma_[A-Za-z0-9_-]{40,}$/.test(String(apiKey || ""))) return null;
  const digest = crypto.createHash("sha256").update(apiKey).digest("hex");
  for (const user of await listUsers()) {
    if (user.apiKeyHash && user.apiKeyHash.length === digest.length && crypto.timingSafeEqual(Buffer.from(user.apiKeyHash), Buffer.from(digest))) return user;
  }
  return null;
}
...
export async function consumeCredit(userId) {
  ...
  if (!user) return { allowed: false, credits: 0 };
  const credits = Number.isInteger(user.credits) ? user.credits : 100;
  if (credits <= 0) return { allowed: false, credits: 0 };
...
export async function uploadStudentResource(userId, file) {
  const { service } = await storage();
  const resourceId = crypto.randomUUID();
  const name = safePart(file.name);
  const blobName = `${userId}/${resourceId}/${name}`;
  const blob = service.getContainerClient("student-resources").getBlockBlobClient(blobName);
  await blob.uploadData(Buffer.from(await file.arrayBuffer()), {
    blobHTTPHeaders: { blobContentType: file.type || "application/octet-stream" },
    metadata: { ownerid: userId, resourceid: resourceId, originalname: encodeURIComponent(file.name), uploadedat: new Date().toISOString() }
  });
  return { id: resourceId, name: file.name, size: file.size, contentType: file.type || "application/octet-stream", uploadedAt: new Date().toISOString() };
}

export async function listStudentResources(userId) {
  const { service } = await storage();
  const container = service.getContainerClient("student-resources");
  const resources = [];
  for await (const item of container.listBlobsFlat({ prefix: `${userId}/`, includeMetadata: true })) {
    const metadata = item.metadata || {};
    if (metadata.ownerid !== userId) continue;
    resources.push({ id: metadata.resourceid || item.name.split("/")[1], name: decodeURIComponent(metadata.originalname || item.name.split("/").at(-1)), size: item.properties.contentLength || 0, contentType: item.properties.contentType || "application/octet-stream", uploadedAt: metadata.uploadedat || item.properties.createdOn, url: `/api/resources/${metadata.resourceid || item.name.split("/")[1]}` });
  }
  return resources.sort((a, b) => new Date(b.uploadedAt) - new Date(a.uploadedAt));
}

export async function getStudentResource(userId, resourceId) {
  const { service } = await storage();
  const container = service.getContainerClient("student-resources");
  for await (const item of container.listBlobsFlat({ prefix: `${userId}/${resourceId}/`, includeMetadata: true })) {
    if (item.metadata?.ownerid !== userId) continue;
    const blob = container.getBlobClient(item.name);
    return { data: await blob.downloadToBuffer(), contentType: item.properties.contentType || "application/octet-stream", name: decodeURIComponent(item.metadata?.originalname || item.name.split("/").at(-1)) };
  }
  return null;
}

export async function listAllStudentResources() {
  const { service } = await storage();
  const container = service.getContainerClient("student-resources");
  const resources = [];
  for await (const item of container.listBlobsFlat({ includeMetadata: true })) {
    const metadata = item.metadata || {};
    if (!metadata.ownerid) continue;
    resources.push({ id: metadata.resourceid || item.name.split("/")[1], ownerId: metadata.ownerid, name: decodeURIComponent(metadata.originalname || item.name.split("/").at(-1)), size: item.properties.contentLength || 0, contentType: item.properties.contentType || "application/octet-stream", uploadedAt: metadata.uploadedat || item.properties.createdOn });
  }
  return resources.sort((a, b) => new Date(b.uploadedAt) - new Date(a.uploadedAt));
}
